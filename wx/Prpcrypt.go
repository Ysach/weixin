...
func (p *Prpcrypt) Encrypt(sCorpID string, src []byte) (ret string, err error) {
...
func (p *Prpcrypt) Decrypt(sCorpID string, src string) (ret []byte, err error) {
	b, err := aes.NewCipher(p.key)
	if err != nil {
		err = DecryptAESError
		return
	...
	fromsCorpID := string(content[xmlLen+4:])
	if sCorpID != fromsCorpID {
